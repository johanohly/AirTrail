<script lang="ts">
	...
	import { Area, Axis, Bars, Chart, LinearGradient, Points, Svg, Tooltip, TooltipItem } from 'layerchart';
	import { scaleBand } from 'd3-scale';
	import { curveBasis, curveCardinal, curveCatmullRom, curveNatural } from 'd3-shape';
	...
	const flightsPerWeekday = $derived.by(() => {
		...
		return weekdays.map((weekday) => {
			const weekdayFlights = data.filter((flight) => flight.departure && dayjs.unix(flight.departure).day() === weekday);
			return {
				weekday,
...
<div class="w-full h-[200px] p-4 border rounded">
	...
	>
		<Svg>
			<Axis placement="left" grid rule ticks={(scale) => scale.ticks?.().filter(Number.isInteger)} format="integer" />
			<Axis placement="bottom" format={(i) => {
				return WEEKDAYS[i] ?? '';
			}} rule />
