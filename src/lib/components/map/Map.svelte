<script lang="ts">
  ...
  const visitedAirports = $derived.by(() => {
    ...
    data.forEach((trip) => {
      ...
      if (!visited.includes({ position: trip.from.position, name: trip.from.name }))
        visited.push({ position: trip.from.position, name: trip.from.name });
      if (!visited.includes({ position: trip.to.position, name: trip.to.name }))
        visited.push({ position: trip.to.position, name: trip.to.name });
    });
...
>
  ...
  <DeckGlLayer
    ...
    greatCircle={true}
  >
    <Popup openOn="hover" anchor="top-left" offset={12} let:data let:close>
